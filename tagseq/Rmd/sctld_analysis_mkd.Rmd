...
```{r}
ggplot(data = ofav_DEG_GLM_treatment, 
            aes(x = logFC, 
...
```
```{r}
# save p-value tables of DEG from GLM with treatment
...
ofav_DEG_GLM_treatment_locus <- as.data.frame(add_rownames(ofav_DEG_GLM_treatment, var = "locus"))

```

```{r}
ofav_annot_DEG <- left_join(ofav_DEG_GLM_treatment_locus, ofav_prot, by="locus")
```
...
goAnnotations <-data.frame(ofav_annot_DEG_emap$locus, ofav_annot_DEG_emap$GOs)

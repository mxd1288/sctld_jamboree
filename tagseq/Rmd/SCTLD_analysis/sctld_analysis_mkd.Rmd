...
```
Check convergence of iterations, last two should be within 0.01
```{r}
EBOut$Alpha
...
```{r, fig.width=10}
# Plots for ofav
df_allnorm <- rbind(df_raw_ofav, df_deseq, df_RPKM, df_TC, df_TMM, df_MRN)
df_allnorm$method <- factor(df_allnorm$method,
                            levels = c("Raw counts", "DESeq (RLE)", "RPKM","TC", "TMM", "MRN"))
...
BCV <- plotBCV(cur_dge, main = paste0("BCV plot"))
...
#ggsave("hist_GLM_pvals.jpeg", p, width=10, height=8)
```
...
# subset of mcav data
```{r}
dds_mcav_mote <- dds_mcav[, colData(dds_mcav)$Project != "Smithsonian"]
mcav_mote_meta <- subset(mcav_meta_dd, mcav_meta_dd$Project != "Smithsonian")
```

```{r}
which(colnames(raw_counts_mcav) %in% mcav_mote_meta$Working.Label)
raw_counts_mcav_mote <- data.frame(raw_counts_mcav[ which(colnames(raw_counts_mcav) %in% mcav_mote_meta$Working.Label)])
```


```{r}
cur_dge <- DGEList(raw_counts_mcav_mote, group = mcav_mote_meta$Correct_treatment)
cur_dge <- calcNormFactors(cur_dge, method = "TMM")

group <- relevel(as.factor(mcav_mote_meta$Correct_treatment), ref = "control")
design_matrix <- model.matrix(~ group)
design_matrix <- design_matrix[,1:2]
...
for (ind in 1) {
  ...
  print(cur_res)
  sel_deg <- which(cur_res[ ,1] != 0)
  cur_res <- cbind(rownames(raw_counts_mcav_mote)[sel_deg], cur_res[sel_deg,1],
                   rep(all_conditions[ind], length(sel_deg)))
  DEG_GLM1 <- rbind(DEG_GLM1, cur_res)
}
...
pvals_GLM1_mcav <- data.frame("pvalue" = pvals_GLM1,
                         "type" = rep(rep(c("raw", "adjusted"),
                                          each = nrow(raw_counts_mcav_mote)),
                                      length(all_conditions)),
                         "condition" = rep(all_conditions,
                                           each=nrow(raw_counts_mcav_mote)*2))

mcav_mote_DEG_GLM_treatment <- as.data.frame(res_GLM1$table)
#FDR correction
mcav_mote_DEG_GLM_treatment$PAdju_BH <- p.adjust(res_GLM1$table$PValue, method="BH")
#BONF correction
mcav_mote_DEG_GLM_treatment$PAdju_bonf <- p.adjust(res_GLM1$table$PValue, method="bonferroni")

# save sign from FDR and BONF conrrections
# 61 genes
mcav_mote_DEG_GLM_treatment_sign_BH <- subset(mcav_mote_DEG_GLM_treatment, mcav_mote_DEG_GLM_treatment$PAdju_BH <0.05)
# 8 genes
mcav_mote_DEG_GLM_treatment_sign_bonf <- subset(mcav_mote_DEG_GLM_treatment, mcav_mote_DEG_GLM_treatment$PAdju_bonf <0.05)
```
...
#ggsave("hist_GLM_pvals_mcav.jpeg", p, width=10, height=8)
```

```{r}
#write.csv(mcav_mote_DEG_GLM_treatment, "mcav_mote_DEG_GLM_treatment.csv")
```

```{r}
mcav_smit_meta <- subset(mcav_meta_dd, mcav_meta_dd$Project == "Smithsonian")
which(colnames(raw_counts_mcav) %in% mcav_smit_meta$Working.Label)
raw_counts_mcav_smit <- data.frame(raw_counts_mcav[ which(colnames(raw_counts_mcav) %in% mcav_smit_meta$Working.Label)])
```

```{r}
cur_dge <- DGEList(raw_counts_mcav_smit, group = mcav_smit_meta$Correct_treatment)
cur_dge <- calcNormFactors(cur_dge, method = "TMM")

group <- relevel(as.factor(mcav_smit_meta$Correct_treatment), ref = "control")
design_matrix <- model.matrix(~ group)
design_matrix <- design_matrix[,1:2]
...
mcav_smit_DEG_GLM_treatment <- as.data.frame(res_GLM1$table)
#FDR correction
mcav_smit_DEG_GLM_treatment$PAdju_BH <- p.adjust(res_GLM1$table$PValue, method="BH")
#BONF correction
mcav_smit_DEG_GLM_treatment$PAdju_bonf <- p.adjust(res_GLM1$table$PValue, method="bonferroni")

# save sign from FDR and BONF conrrections
# 61 genes
mcav_smit_DEG_GLM_treatment_sign_BH <- subset(mcav_smit_DEG_GLM_treatment, mcav_smit_DEG_GLM_treatment$PAdju_BH <0.05)
# 8 genes
mcav_smit_DEG_GLM_treatment_sign_bonf <- subset(mcav_smit_DEG_GLM_treatment, mcav_smit_DEG_GLM_treatment$PAdju_bonf <0.05)
```
...
```{r}
#write.csv(mcav_smit_DEG_GLM_treatment, "mcav_smit_DEG_GLM_treatment.csv")
```
